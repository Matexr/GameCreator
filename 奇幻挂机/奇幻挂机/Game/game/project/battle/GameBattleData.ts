...
class GameBattleData {
    ...
    static addStatus(targetBattler: Battler, statusID: number, fromBattler: Battler = null, force: boolean = false): boolean {
        ...
        // -- 事件处理
        EventUtils.happen(GameBattleData, GameBattleData.EVENT_CALC_HIT_RESULT_OVER, [fromBattler, targetBattler, statusID, thisStatus, force]);
        if (thisStatus.eventSetting && thisStatus.addEvent) CommandPage.startTriggerFragmentEvent(thisStatus.addEvent, fromBattler, targetBattler);
        return true;
    }
    /**
     * 移除目标状态
    ...
    static removeStatus(targetBattler: Battler, statusID: number): boolean {
        ...
        if (thisStatusIdx != null) {
            ...
            // -- 事件处理
            EventUtils.happen(GameBattleData, GameBattleData.EVENT_CALC_HIT_RESULT_OVER, [targetBattler, statusID, removedStatus]);
    ...
    static removeAllStatus(battler: Battler): void {
        ...
        for (let i = 0; i < statusArr.length; i++) {
            ...
            // -- 事件处理
            EventUtils.happen(GameBattleData, GameBattleData.EVENT_CALC_HIT_RESULT_OVER, [battler, status.id, status]);
