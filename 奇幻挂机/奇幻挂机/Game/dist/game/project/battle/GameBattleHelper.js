var GameBattleHelper = (function () {
    ...
    GameBattleHelper.canAttack = function (battler) {
        ...
        if (actor.atkMode == 1 && (!actor.atkSkill || actor.atkSkill.currentCD != 0 || actor.atkSkill.costSP > battler.actor.sp || actor.atkSkill.costHP >= battler.actor.hp))
            return false;
    ...
    GameBattleHelper.getCounterattackDamagePer = function (fromBattler, targetBattler) {
        for (var i = 0; i < fromBattler.actor.skills.length; i++) {
            var skill = fromBattler.actor.skills[i];
            if (skill.specialAbility && ArrayUtils.matchAttributes(skill.specialBattleEffect, { type: 1 }, true).length == 1) {
                return null;
            }
        }
        for (var i = 0; i < fromBattler.actor.status.length; i++) {
            var status = fromBattler.actor.status[i];
    ...
    GameBattleHelper.getSuckPer = function (fromBattler, suckCondition, isHP) {
        var type = isHP ? 4 : 5;
    ...
    GameBattleHelper.getSpecialBattleEffects = function (battler, specialType) {
        var specialBattleEffects = [];
        for (var i = 0; i < battler.actor.skills.length; i++) {
            var skill = battler.actor.skills[i];
