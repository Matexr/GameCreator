var GameBattle = (function () {
    ...
    GameBattle.start = function () {
        ...
        new SyncTask(GameBattle.battleStartTask, function () {
            ...
            var battleStage0_inBattle = _this.enemyParty.eventSetting ? _this.enemyParty.battleStage0_inBattle : null;
            CommandPage.startTriggerFragmentEvent(battleStage0_inBattle, Game.player.sceneObject, Game.player.sceneObject, Callback.New(function () {
                if (!_this.nextStepCB)
                    _this.nextStepCB = Callback.New(_this.nextStep, _this);
                GameBattle.state = 2;
    ...
    GameBattle.nextStep = function () {
        ...
        this.battlerfieldDetermineHandle(function () {
            ...
            if (currentInTurnStage == 0) {
                ...
                _this.playerControlStep();
            }
            else if (currentInTurnStage == 2) {
                _this.battlerActionStep();
            }
            else {
                _this.settlementStep();
            }
            _this.battleUI.refreshPlayerActorPanel();
        });
    };
        ...
        if (this.enemyParty.battleStage1_newTurn) {
            ...
            CommandPage.startTriggerFragmentEvent(battleStage1_newTurn, Game.player.sceneObject, Game.player.sceneObject, Callback.New(function () {
                _this.battlerfieldDetermineHandle(function () {
                    GameCommand.startCommonCommand(14024, [], Callback.New(_this.nextStep, _this), Game.player.sceneObject, Game.player.sceneObject);
                });
            }, this));
        }
        else {
    ...
    GameBattle.battlerActionStep = function () {
        ...
        if (this.enemyParty.battleStage2_beforeAction) {
            ...
            CommandPage.startTriggerFragmentEvent(battleStage2_beforeAction, Game.player.sceneObject, Game.player.sceneObject, Callback.New(function () {
                _this.battlerfieldDetermineHandle(function () {
                    GameCommand.startCommonCommand(14025, [], Callback.New(_this.battlersAction, _this), Game.player.sceneObject, Game.player.sceneObject);
                });
            }, this));
        }
        else {
                ...
                this.checkBattlerIsDead(battler, function () {
                    ...
                    if (allBattlersCount == 0) {
                        ...
                        if (!isComplete)
                            ...
                            onBattlerOver && onBattlerOver.apply(_this);
                    }
                });
            }
        }
                ...
                GameBattleAction.releaseAction(battler, 7, Number.MAX_VALUE, null, function () {
                    ...
                    if (needRemoveEnemy_1) {
                        ...
                        GameCommand.startCommonCommand(14028, [], Callback.New(function () {
                            ...
                            if (deadEventSign == _this.deadEventSign)
                                SyncTask.taskOver(GameBattle.battleOverTask);
                        }, _this), battler, battler);
                    }
                    else {
                        if (deadEventSign == _this.deadEventSign)
                            SyncTask.taskOver(GameBattle.battleOverTask);
    ...
    GameBattle.checkBattleIsComplete = function () {
        ...
        if (isBattleOver) {
            ...
            setTimeout(function () {
                _this.calcReward(Callback.New(function () {
                    var battleStage3_outBattle = _this.enemyParty.eventSetting ? _this.enemyParty.battleStage3_outBattle : null;
    ...
    GameBattle.initBattlefield = function () {
        var _this = this;
        new SyncTask(GameBattle.battleStartTask, function () {
            _this.battleUI.createBattleScene(function () {
                _this.lastCurrentScene = Game.currentScene;
                Game.currentScene = _this.battleUI.currentBattleScene;
                _this.enemyBattlers.length = 0;
                _this.playerBattlers.length = 0;
                _this.removedEnemyBattlers.length = 0;
                GameBattleData.initEnemyPartyPackageItem(_this.enemyParty);
                var enemys;
                ...
                for (var i = 0; i < 99; i++) {
                    var enemyRefBattlerUI = _this.battleUI.getRefEnemyBattlerUI(i);
                    ...
                    if (i >= enemyLength) {
                        ...
                        _this.enemyBattlers.push(enemyBattler);
                    }
                }
                for (var i = 0; i < 99; i++) {
                    var playerRefBattlerUI = _this.battleUI.getRefPlayerBattlerUI(i);
                    ...
                    if (i >= Game.player.data.party.length) {
                        ...
                        _this.playerBattlers.push(playerBattler);
                    }
                }
                _this.battleUI.init();
                SyncTask.taskOver(GameBattle.battleStartTask);
